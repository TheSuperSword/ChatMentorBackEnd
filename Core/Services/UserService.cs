...
public class UserService
{
    ...
    {
        if (page <= 0 || pageSize <= 0) throw new ArgumentException("Page and pageSize must be greater than 0.");
        ...
        if (page > totalPages && totalRecords > 0) throw new ArgumentException("Page number is out of range.");
        ...
        foreach (var user in paginatedUsers) userDtos.Add(await MapToDtoAsync(user));
